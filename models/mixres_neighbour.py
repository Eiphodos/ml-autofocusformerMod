...
class ClusterAttention(nn.Module):
    ...
    def forward(self, feat, member_idx, cluster_mask, pe_idx, global_attn):
        ...
        if not global_attn:
            ...
            key, v = kv[0], kv[1]
            attn = CLUSTENQKFunction.apply(q.long(), key.long(), member_idx.long())  # b x h x n x m
        ...
        if global_attn:
            ...
            feat = CLUSTENAVFunction.apply(attn.long(), v.long(), member_idx.long()).permute(0, 2, 1, 3).reshape(b, n, c)
            feat = feat.to(orig_dtype)
            feat = feat + blank_v.permute(0, 2, 1, 3).reshape(b, n, c)
